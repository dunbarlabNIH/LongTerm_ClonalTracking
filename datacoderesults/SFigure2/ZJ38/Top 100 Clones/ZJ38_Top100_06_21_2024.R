...
#read in data
nonMatrixHeatData <- read.delim("ZJ38_HeatmapData_Top100Clones.txt")
...
Gr_xAxisNames <- read.delim("ZJ38_Gr_GraphNames.txt", header = F)
...
write.table(combined_df,"Gr_matrix_ZJ38_Top100.txt",sep="\t",row.names=FALSE)

rightOrder <- c("9.5m vs. 12m","12m vs. 21m","21m vs. 24m","24m vs. 44m",
                "44m vs. 64m","64m vs. 99m", "99m vs. 117m")
