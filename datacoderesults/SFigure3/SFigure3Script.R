...
dir.create(dirname)
setwd(dirname)
#make top clone heatmaps for T cells
BCLH(data = zh33alldata[,as.vector(zh33indexmatrixnoNA[,1])], names = zh33timepoints,n_clones = 100, folder = "ZH33 T")
BCLH(data = zg66alldata[,as.vector(zg66indexmatrixnoNA[,1])], names = zg66timepoints,n_clones = 100, folder = "ZG66 T")
BCLH(data = zh19alldata[,as.vector(zh19indexmatrixnoNA[,1])], names = zh19timepoints,n_clones = 100, folder = "ZH19 T")
BCLH(data = zj31alldata[,as.vector(zj31indexmatrixnoNA[,1])], names = zj31timepoints,n_clones = 100, folder = "ZJ31 T")
BCLH(data = zj38alldata[,as.vector(zj38indexmatrixnoNA[,1])], names = zj38timepoints,n_clones = 100, folder = "ZJ38 T")
setwd(workingdir)