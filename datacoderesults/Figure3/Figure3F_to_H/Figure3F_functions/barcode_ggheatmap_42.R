...
barcode_ggheatmap <- function(your_SE,
    ...
    if (row_order == "hierarchical" | row_order == "emergence") {
        ...
        # Error handling for if the number of non-zero clones is less than n_clones
        bad_samples <- apply(SummarizedExperiment::assays(your_SE)$ranks, 2, function(x) {
            any(max(x) < n_clones, na.rm = TRUE)
        })
        if (sum(bad_samples) > 0){
            message(paste("Samples with less non-zero barcodes than specified n_clones:", colnames(your_SE)[bad_samples]))
        }
        # for all samples, set rank of zero abundance barcodes to Inf
        SummarizedExperiment::assays(your_SE)$ranks[SummarizedExperiment::assays(your_SE)$counts == 0] <- Inf
