...
dir.create(dirname)
setwd(dirname)
#make top clone heatmaps for B cells
BCLH(data = zh33alldata[,as.vector(zh33indexmatrixnoNA[,2])], names = zh33timepoints,n_clones = 100, folder = "ZH33 B")
BCLH(data = zg66alldata[,as.vector(zg66indexmatrixnoNA[,2])], names = zg66timepoints,n_clones = 100, folder = "ZG66 B")
BCLH(data = zh19alldata[,as.vector(zh19indexmatrixnoNA[,2])], names = zh19timepoints,n_clones = 100, folder = "ZH19 B")
BCLH(data = zj31alldata[,as.vector(zj31indexmatrixnoNA[,2])], names = zj31timepoints,n_clones = 100, folder = "ZJ31 B")
BCLH(data = zj38alldata[,as.vector(zj38indexmatrixnoNA[,2])], names = zj38timepoints,n_clones = 100, folder = "ZJ38 B")
setwd(workingdir)